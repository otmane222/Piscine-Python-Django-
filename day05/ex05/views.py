...
def display(request):
    with connection.cursor() as cursor:
        cursor.execute("SELECT episode_nb, title, opening_crawl, director, producer, release_date FROM ex05_movies")
    ...
    return render(request, 'ex05/display.html', {'movies': data})
...
def populate(request):
    ...
    try:    
        ...
        return render(request, 'ex05/populate.html', {'state': state})
    return render(request, 'ex05/populate.html', {'state': state})
...
def remove(request):
    if (request.method == 'POST'):
        ...
        movie = Movies.objects.get(title=titlo)
        movie.delete()
